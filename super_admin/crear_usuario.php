...
if (isset($_POST['crear'])) {
    ...
    elseif ($documento == "" || $nombre == "" || $correo == "" || $telefono == "" || $tipo_documento == "" || $passw == "" || $id_estado == ""|| $nit_biblioteca == "") {
        echo '<script>alert("Existen datos vacios")</script>';
        echo '<script>window.location="crear_usuario.php"</script>';
        exit();
    } else {
        $sql = "INSERT INTO usuarios (documento, nombre, correo, telefono, tipo_documento, id_estado, nit_biblioteca, passw, id_rol)
                VALUES ('$documento', '$nombre', '$correo', '$telefono', '$tipo_documento', '$id_estado', '$nit_biblioteca', '$hashed_password', 1)";
        $con->query($sql);
    ...
    <form action="" method="post">
        <table border="2">
            ...
            <tr>
                <td>biblioteca</td>
                <td>
                    <select name="nit_biblioteca" required>
                      <option value="">Seleccione biblioteca</option>
                        <?php
                        $sql = "SELECT * FROM bibliotecas";
                        $result = $con->query($sql);
                        while ($fila = $result->fetch(PDO::FETCH_ASSOC)) {
                            echo "<option value='" . $fila['nit_biblioteca'] . "'>" . $fila['nom_biblioteca'] . "</option>";
                        }
                        ?>
                    </select>
                </td>
